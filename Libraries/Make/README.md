...
`oct make explain [target] [--file <path>]` reports the selected target closure and current staleness reasons without executing targets or mutating state. `oct make doctor [--file <path>]` reports make health: profile, state directory, backend, default target, target counts, validation/dependency status, state/trace existence, referenced programs, and advisory Make attribute diagnostics. Doctor reports conventional versus `[MakePlan]` entrypoint status, lists explicit `[Pure]`, `[NoWhile]`, and `[RequiresAuthority]` markers, reports `[RequiresAuthority]` OK for direct Make host primitive calls in marked functions, and suggests typed `Make.Tool`/`Make.Env` idioms for obvious shell-shaped `Make.Exec("bash", ["-c", ...])` probes. Missing `[RequiresAuthority]` on a `Make.oct` function that directly calls a Make host primitive is now a load/typecheck error, so doctor may fail before printing advisory output for invalid files. The authority rule is direct-call-only; transitive helper analysis and a broader `[Pure]` effect checker are deferred. Ordinary `.oct` files still do not use Make attributes.
...
MAKE4 uses the narrow `Int` result convention: a completed flow returning `0` succeeds, and a completed flow returning any non-zero integer fails the target. `FunctionTarget` also treats a nonzero `Int` return as target failure while preserving ordinary Oct error failures. `MaxSteps` must be positive and bounds flow transitions so accidental non-terminating flows fail clearly. `trace.octagon` records `MaxSteps` as the configured cap and `Steps` as the actual executed state count. If a flow intentionally suspends before completion, the target fails with a diagnostic naming the suspended state and explaining that persistent make-flow resume is not supported yet; failure artifacts and trace decisions classify this with `SuspendedIntentionally`. Persistent resume remains a future checkpointing/invalidation design item. `trace.octagon` records flow name, max steps, executed steps, state history, result code when available, suspended status, and errors; `state.octagon` records the target as kind `flow` with status and path state.
