...
Still unsupported/deferred after M18: general record returns, nested or recursive records, cross-family handles, handle close/destructor semantics, handle serialization across runs, dynamic `Any`, maps, `Float[][]`, broad `Int[]`, Image/Pdf migration, structured JSON graph helpers, Markdown-as-Octxiliary, package-manager sidecar builds, native permission prompts, and lockfiles. The `octxiliary-plot` and `octxiliary-xlsx` executables must be beside the compiled `.octbin` or on `OCT_WRAPPER_PATH`. M18 adds handle transport M0 and migrates `IO.Xlsx`; handles are sidecar-owned capabilities, not plain `Int` values.
