...
| `Libraries/Hash` | pass | pass | fail, 0 / 3 | `unsupported_builtin` / `unsupported_wrapper` | `Sha256Bytes`, `Sha256File`, `Sha256Text` | Hash builtins are not compiled; good candidate non-IO wrapper proof. | M6/M7 |
| `Libraries/IO` | pass | pass | fail, 1 / 34 | `unsupported_builtin`, `sidecar_environment`, `missing_manifest`, `generated_go_error` | CSV/JSON/XLSX builtins; file/directory round trips; bytes round trip | File/directory wrappers reach compiled runtime but panic when `OCT_WRAPPER_PATH`/`octxiliary-io` is unavailable. CSV/JSON/XLSX still unsupported. Bytes alias emits undefined `IO_Bytes`. | M6, M9, M10 |
...
| `Libraries/Image` | fail | fail | fail, 0 / 5 | `unsupported_builtin`, test root/assets | `ImageLoad` | Interpreted also fails because fixture images are not found from this root; compiled additionally lacks image builtins. | M6/M9 |
...
| `Image` | load/save/metadata | fail | `wrapper_registry_generic_octxiliary` | image builtins unsupported; fixture root | medium |
