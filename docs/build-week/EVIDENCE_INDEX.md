...
| Direct GGML/SPIR-V comparison and packed short rows | uncommitted submission refinement | current Codex task | `tools/vulkan_shader_comparison`; packed reduction shaders; reactor registry/plan | direct raw artifact, `spirv-val`, native reduction tests | Same-harness GGML comparison exposed a short-row physical boundary; one bounded packed plan materially closed it. | Matmul remains an explicit cooperative-matrix ABI seam; no matmul claim is made. |
