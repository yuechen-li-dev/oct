...
func TestDispatchResponseRoundTrip(t *testing.T) {
	response := kaijuvulkan.DispatchResponse{
		...
		ReplayID: "replay-id",
		SpirvSHA256: "hash",
		Device: kaijuvulkan.DeviceInfo{RuntimeName: "sidecar", RuntimeVersion: "v1", DeviceName: "gpu", VendorID: 1, DeviceID: 2, DriverVersion: 3, VulkanAPIVersion: "1.0", TimestampPeriodNS: 4.5, TimestampValidBits: 64, Headless: true},
		Timing: kaijuvulkan.Timing{Source: kaijuvulkan.TimingSourceVulkanQueryPoolGPU, StageSpan: kaijuvulkan.TimingStageBoundsComputeDispatch, SamplesNS: []uint64{10, 12}},
	...
	if !got.Success || got.ReplayID != response.ReplayID || got.Device.DeviceName != response.Device.DeviceName || len(got.Timing.SamplesNS) != 2 || len(got.Readbacks) != 1 || len(got.Warnings) != 1 {
