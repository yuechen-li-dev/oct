...
func main() {
	...
	layer := flag.Uint("layer", 0, "closed MainTransformer layer index (0..29)")
	flag.Parse()
	if *source == "" || *root == "" {
		flag.Usage()
		os.Exit(2)
	}
	manifest, inventory, err := zimage.BuildMainTransformerLayerCache(*source, *root, *sha, uint32(*layer))
	if err != nil {
	...
	if err := os.WriteFile(filepath.Join(zimage.MainTransformerLayerCacheRoot(*root, uint32(*layer)), "tensor_inventory.json"), encoded, 0o644); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
