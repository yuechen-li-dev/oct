...
static int prom_ray_build_tlas(prom_ray_query_scene* scene) {
  ...
  VkAccelerationStructureBuildGeometryInfoKHR build_info;
  VkAccelerationStructureBuildRangeInfoKHR range;
  uint32_t primitive_count = 1u;
  VkDeviceAddress instance_address;
  if (scene == NULL || scene->triangle_blas.device_address == 0u) return 0;
  ...
  instance.accelerationStructureReference = scene->triangle_blas.device_address;
  ...
  memcpy(scene->instance_buffer.mapped, &instance, sizeof(instance));
